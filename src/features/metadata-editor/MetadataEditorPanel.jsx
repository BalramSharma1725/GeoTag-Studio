...
export default function MetadataEditorPanel({ onOpenMap, showToast }) {
  ...
  useEffect(() => {
    if (img) {
      setLat(img.editedGps ? img.editedGps.lat.toString() : String(DEFAULT_LAT))
      setLng(img.editedGps ? img.editedGps.lng.toString() : String(DEFAULT_LNG))
      setKeywords(img.keywords || '')
      setDescription(img.description || '')
    } else {
      setLat(String(DEFAULT_LAT)); setLng(String(DEFAULT_LNG)); setKeywords(''); setDescription('')
    }
    setErrors({})
  }, [activeId, img])
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const center = img && img.editedGps ? [img.editedGps.lat, img.editedGps.lng] : [DEFAULT_LAT, DEFAULT_LNG]
  ...
  const handleSaveMeta = () => {
    if (!img) return
    updateMeta(img.id, { keywords: keywords.trim(), description: description.trim() })
    showToast('Metadata saved for ' + img.name, 'success')
  }

  if (!img) {
    return (
      <div className='flex flex-col items-center justify-center h-full text-base text-center px-6 gap-2' style={{ color: 'var(--color-muted)' }}>
  ...
  return (
    <motion.div {...slideRight} className='h-full overflow-y-auto px-4 py-4 space-y-4'>
      ...
      <section aria-label="Image preview">
        ...
        />
        <div className='text-sm truncate' style={{ color: 'var(--color-muted)' }}>{img.name}</div>
        <div className='font-mono text-[13px]' style={{ color: 'var(--color-border2)' }}>{formatFileSize(img.size)}</div>
        ...
        {hasOriginalGps && !isEdited && (
          <div className='mt-2 px-2 py-1.5 rounded-lg flex items-center gap-2' style={{ background: 'rgba(234,179,8,0.1)', border: '1px solid rgba(234,179,8,0.3)' }}>
            <span>⚠️</span>
            <span className='text-sm font-medium' style={{ color: '#eab308' }}>Already geotagged</span>
          </div>
        )}
      </section>
      ...
      <section aria-label="Place search">
        ...
        <div className='flex gap-1.5'>
          <input
            ...
            id="place-search"
          />
          <button onClick={handleSearch} disabled={searching} className='btn btn-primary text-sm px-3 py-1.5' aria-label="Search location">
            {searching ? '...' : '🔍'}
          </button>
        </div>
      </section>
      ...
      <section aria-label="Existing geotags">
        ...
        <div className='rounded-lg px-3 py-2 font-mono text-sm space-y-1'
      ...
      <section aria-label="GPS coordinates">
        ...
        <div className='grid grid-cols-2 gap-2 mb-2'>
          <div>
            <label className='text-sm block mb-1' style={{ color: 'var(--color-muted)' }} htmlFor="lat-input">Latitude (-90 to 90)</label>
            <input
              id="lat-input"
              type='number'
              className={'input-field text-sm py-1.5' + (errors.lat ? ' error' : '')}
              value={lat}
            ...
            />
            {errors.lat && <div className='text-sm mt-1' style={{ color: '#f87171' }} role="alert">{errors.lat}</div>}
          </div>
          <div>
            <label className='text-sm block mb-1' style={{ color: 'var(--color-muted)' }} htmlFor="lng-input">Longitude (-180 to 180)</label>
            <input
              id="lng-input"
              type='number'
              className={'input-field text-sm py-1.5' + (errors.lng ? ' error' : '')}
              value={lng}
            ...
            />
            {errors.lng && <div className='text-sm mt-1' style={{ color: '#f87171' }} role="alert">{errors.lng}</div>}
          </div>
        </div>
        <div className='flex gap-1.5 mb-2' role="radiogroup" aria-label="Apply scope">
          {SCOPES.map((s) => (
            <button key={s.id} onClick={() => setScope(s.id)}
              className='flex-1 py-1 px-1 rounded-lg border text-sm transition-all duration-150'
              style={{
        ...
        <div className='flex gap-2'>
          {editedGps && (
            <button onClick={() => { resetGps(img.id); showToast('GPS reset', 'success') }}
              className='btn btn-danger btn-sm flex-1 justify-center text-sm'>Reset</button>
          )}
          {hasUndo && (
            <button onClick={() => { undoGps(img.id); showToast('Undone', 'success') }}
              className='btn btn-ghost btn-sm flex-1 justify-center text-sm'>Undo</button>
          )}
        </div>
      </section>
      ...
      <section aria-label="Keywords">
        ...
        <textarea
          ...
          id="keywords-input"
        />
        <div className='text-sm text-right mt-0.5' style={{ color: 'var(--color-border2)' }}>{keywords.length}/6600</div>
      </section>

      <section aria-label="Description">
        <div className='section-label'>Description</div>
        <textarea
          className='input-field text-sm resize-none w-full mb-1'
          rows={3}
        ...
        />
        <div className='text-sm text-right mb-2' style={{ color: 'var(--color-border2)' }}>{description.length}/1300</div>
      </section>
          ...
          {[{ label: 'Before', gps: origGps, color: 'var(--color-muted)' }, { label: 'After', gps: editedGps, color: 'var(--color-accent3)' }]
            .map(({ label, gps, color }) => (
              <div key={label} className='rounded-lg p-2.5'
                style={{ background: 'var(--color-surface2)', border: '1px solid var(--color-border)' }}
              >
                <div className='font-mono text-sm uppercase tracking-wider mb-2' style={{ color: 'var(--color-border2)' }}>{label}</div>
                {gps ? (
                ...
                ) : <div className='text-sm italic' style={{ color: 'var(--color-border2)' }}>No GPS</div>}
              </div>
            ))}
        </div>
      </section>
