...
export default function MapPicker({ isOpen, initialCenter, onApply, onClose }) {
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      const m = L.map(mapRef.current, { center, zoom: initialCenter ? 10 : 12 })
          ...
          >
            <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid var(--color-border)' }}>
              <div>
                ...
                <div className="text-[14px] mt-0.5" style={{ color: 'var(--color-muted)' }}>Click anywhere on the map to set GPS coordinates</div>
              </div>
              <button onClick={onClose} className="btn btn-ghost text-[14px]" aria-label="Close map picker">✕ Close</button>
            </div>
            ...
            >
              <div className="flex-1 font-mono text-[14px]" style={{ color: 'var(--color-muted)' }}>
                {picked ? (
              ...
              <select
                ...
                className="text-[14px] px-2 py-1.5 rounded-lg cursor-pointer outline-none"
                style={{
