  ...
  useEffect(() => {
    if (isOpen) {
      if (images.length > 0) {
        ...
        setGlobalLat(firstWithGps?.editedGps ? firstWithGps.editedGps.lat : DEFAULT_LAT)
        setGlobalLng(firstWithGps?.editedGps ? firstWithGps.editedGps.lng : DEFAULT_LNG)
      } else {
        setGlobalLat(DEFAULT_LAT)
        setGlobalLng(DEFAULT_LNG)
      }
      const initial = {}
        ...
        >
          ...
          <div className="px-5 py-4 flex items-center justify-between flex-shrink-0" style={{ borderBottom: '1px solid var(--color-border)' }}>
            <div>
              ...
              <div className="text-[14px] mt-0.5" style={{ color: 'var(--color-muted)' }}>
                Apply shared tags + unique ALT text per image
              </div>
            </div>
            <button onClick={onClose} className="btn btn-ghost text-[14px]" aria-label="Close bulk editor">✕ Close</button>
          </div>
          ...
          <div className="px-5 py-4 flex flex-col gap-3 flex-shrink-0" style={{ background: 'var(--color-surface2)', borderBottom: '1px solid var(--color-border)' }}>
            <div className="text-[14px] font-bold uppercase tracking-wider" style={{ color: 'var(--color-text)' }}>
              1. Shared Metadata (All images)
            </div>
            <div className="grid grid-cols-2 gap-4 max-md:grid-cols-1">
              <div>
                <label className="text-[13px] block mb-1 font-mono uppercase tracking-wide" style={{ color: 'var(--color-border2)' }}>Keywords / Tags</label>
                <textarea className="input-field text-[14px] resize-none w-full" rows={2} placeholder="tag1, tag2, tag3" value={globalKeywords} onChange={(e) => setGlobalKeywords(e.target.value)} />
              </div>
              <div>
                <label className="text-[13px] block mb-1 font-mono uppercase tracking-wide" style={{ color: 'var(--color-border2)' }}>Image Description</label>
                <textarea className="input-field text-[14px] resize-none w-full" rows={2} placeholder="Globally applied description..." value={globalDescription} onChange={(e) => setGlobalDescription(e.target.value)} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 max-md:grid-cols-1 mt-1">
              <div>
                <label className="text-[13px] block mb-1 font-mono uppercase tracking-wide" style={{ color: 'var(--color-border2)' }}>Latitude (Optional)</label>
                <input type="number" step="0.000001" min="-90" max="90" className="input-field text-[14px] w-full py-1.5" placeholder="e.g. 40.7128" value={globalLat} onChange={(e) => setGlobalLat(e.target.value)} />
              </div>
              <div>
                <label className="text-[13px] block mb-1 font-mono uppercase tracking-wide" style={{ color: 'var(--color-border2)' }}>Longitude (Optional)</label>
                <input type="number" step="0.000001" min="-180" max="180" className="input-field text-[14px] w-full py-1.5" placeholder="e.g. -74.006" value={globalLng} onChange={(e) => setGlobalLng(e.target.value)} />
              </div>
            </div>
          </div>
          ...
          <div className="px-5 py-2.5 flex items-center gap-2 flex-wrap flex-shrink-0" style={{ borderBottom: '1px solid var(--color-border)' }}>
            <span className="text-[13px] font-bold uppercase tracking-wider" style={{ color: 'var(--color-text)' }}>2. Unique ALT Text</span>
            ...
            {[
              ...
              <button
                ...
                className="text-[13px] px-2.5 py-1 rounded-lg border transition-all"
                style={{
            ...
            <button onClick={handleFillDownAlt} className="text-[12px] border border-border rounded px-1.5 py-0.5 hover:text-text text-muted transition-colors" title="Copy first row ALT text to all">
              ↓ Fill Down
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-1 pb-4">
            {filteredImages.length === 0 ? (
              <div className="p-8 text-center text-base" style={{ color: 'var(--color-muted)' }}>
                <span className="text-3xl block mb-2">🧾</span>
                No images match the current filter
              </div>
            ) : (
              <table className="w-full text-left text-[14px] border-collapse" style={{ color: 'var(--color-text)' }}>
          ...
          <div className="px-5 py-3 flex items-center gap-3 flex-shrink-0" style={{ borderTop: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
            <div className="flex-1" />
            <button onClick={onClose} className="btn btn-ghost text-[14px]">Cancel</button>
            <motion.button
