        ...
        >
          <div className="px-5 py-4 flex items-center justify-between flex-shrink-0" style={{ borderBottom: '1px solid var(--color-border)' }}>
            <div>
              ...
              <div className="text-[12px] mt-0.5" style={{ color: 'var(--color-muted)' }}>Batch import metadata from a CSV or GPX file</div>
            </div>
            <button onClick={onClose} className="btn btn-ghost text-[12px]" aria-label="Close importer">✕</button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-5">
            {step === 'upload' && (
              <div className="space-y-4">
                ...
                >
                  ...
                  <div className="text-[12px] mt-1" style={{ color: 'var(--color-muted)' }}>or click to browse</div>
                ...
                <div className="rounded-xl p-4" style={{ background: 'var(--color-surface2)', border: '1px solid var(--color-border)' }}>
                  <div className="font-mono text-[11px] uppercase tracking-wider mb-2" style={{ color: 'var(--color-accent)' }}>Expected CSV Format</div>
                  <div className="font-mono text-[11px] leading-relaxed" style={{ color: 'var(--color-muted)' }}>
                  ...
                  <div className="mt-2 text-[10px]" style={{ color: 'var(--color-border2)' }}>
                ...
                <button
                  ...
                  className="btn btn-ghost text-[12px] w-full justify-center"
                >
                  ⬇ Download CSV Template
                </button>

                {error && (
                  <div className="bg-red-500/10 border border-red-500/30 rounded-lg px-4 py-3 text-[12px]" style={{ color: '#f87171' }} role="alert">
                    ❌ {error}
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={i} className="rounded-lg p-3 text-center" style={{ background: 'var(--color-surface2)', border: '1px solid var(--color-border)' }}>
                      ...
                      <div className="text-[11px]" style={{ color: 'var(--color-muted)' }}>{s.label}</div>
                    </div>
                  ))}
                </div>
                ...
                {matchedRows.length > 0 && (
                  <div className="rounded-xl overflow-hidden" style={{ background: 'var(--color-surface2)', border: '1px solid var(--color-border)' }}>
                    <div className="px-4 py-2 font-mono text-[10px] uppercase tracking-wider" style={{ borderBottom: '1px solid var(--color-border)', color: 'var(--color-accent)' }}>
                ...
                <div className="flex gap-2">
                  <button onClick={handleReset} className="btn btn-ghost text-[12px]">← Back</button>
                  <motion.button
                    whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    onClick={handleApply}
                    disabled={!matchedRows.length}
                    className="btn btn-primary text-[12px] flex-1 justify-center"
                  >
            ...
            {step === 'done' && (
              <div className="text-center py-8">
                ...
                <div className="text-[13px] mt-1" style={{ color: 'var(--color-muted)' }}>Applied metadata to {matchedRows.length} images</div>
              </div>
            )}
          </div>
        </motion.div>
