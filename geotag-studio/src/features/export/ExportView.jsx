  ...
  return (
    <motion.div {...slideUp} className="flex-1 overflow-y-auto p-6 max-w-2xl">
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[
          ...
          <div key={i} className="rounded-xl p-4 text-center" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
            ...
            <div className="text-[12px] mt-1" style={{ color: 'var(--color-muted)' }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
        ...
        {images.length === 0 ? (
          <div className="p-6 text-center text-sm" style={{ color: 'var(--color-muted)' }}>No images loaded</div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-medium truncate" style={{ color: 'var(--color-text)' }}>{img.name}</div>
                    <div className="font-mono text-[11px] flex gap-2 flex-wrap" style={{ color: 'var(--color-border2)' }}>
                    ...
                    {img.altText && (
                      <div className="font-mono text-[10px] truncate mt-0.5" style={{ color: 'var(--color-muted)' }}>
                        ALT: {img.altText}
                      </div>
                    )}
                  </div>
